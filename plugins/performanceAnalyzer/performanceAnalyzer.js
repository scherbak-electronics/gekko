...
PerformanceAnalyzer.prototype.calculateReportStatistics = function() {
  ...
  console.log('this.start.balance: ' + this.start.balance);
  console.log('relativeProfit: ' + relativeProfit);
  ...
  const sharpe = (relativeYearlyProfit - perfConfig.riskFreeReturn) / statslite.stdev(this.roundTrips.map(r => r.profit)) / Math.sqrt(this.trades / (this.trades - 2));
  const downside = statslite.percentile(this.losses.map(r => r.profit), 0.25) * Math.sqrt(this.trades / (this.trades - 2));
